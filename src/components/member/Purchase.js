...
function Purchase() {
  ...
  useEffect(() => {
    const fetchPurchaseList = async () => {
      try {
        ...
        console.log(
          "[Purchase] Fetching purchase list for memberId:",
          decodedToken.sub
        );
        ...
        if (result.status === 200) {
          console.log("[Purchase] Setting purchase list:", result.data);
          setPurchaseList(result.data || []);
        } else {
          console.error(
            "[Purchase] API returned non-200 status:",
            result.status
          );
          setError(`구매 내역 조회에 실패했습니다. (${result.status})`);
        }
      } catch (error) {
        console.error("[Purchase] Fetch error:", error);
        setError(`구매 내역을 불러오는데 실패했습니다: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  const onClickReviewHandler = (productFromTable) => {
    console.log("[Purchase] Review handler called with:", productFromTable);
